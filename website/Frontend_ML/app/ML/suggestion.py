...
def format_training_and_testing_data(company, timeframe, twitterDays):
    ...
    j = 0
    print("day 0", day)
    print("day end", twitter_sorted_keys[-1])
    print (len(twitter_sorted_keys))
...
def get_suggestion(company):
    ...
    long_response = predict.predict(long_datapoint, company, "long")
    mid_response = predict.predict(mid_datapoint, company, "mid")
    short_response = predict.predict(short_datapoint_arr, company, "short")
